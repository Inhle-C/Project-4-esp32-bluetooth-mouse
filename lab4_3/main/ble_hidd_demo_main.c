...
{
    float a = 1.0;  // Initial acceleration factor
    int x_delta = 0; // Initial delta for x-axis
    int y_delta = 0; // Initial delta for y-axis
    int inclination_level;

    int inclination_start_time = 0;
    int current_time;

    const float max_acceleration = 3.0;   // Limit max acceleration
    const int acceleration_interval = 150; // Increase every 150 ms
    const float acceleration_increment = 0.25;
    const int polling_delay_ms = 200; // Longer delay to reduce polling frequency
    ...
    while (1) {
        ...
        if (sec_conn) {
            inclination_level = read_inclination_level(); 
            current_time = esp_timer_get_time() / 1000;

            switch (inclination_level) {
                case A_BIT_LEFT:
                    x_delta = -10 * a;
                    y_delta = 0;
                    break;
            ...
            if (inclination_level != 0) {
                if (current_time - inclination_start_time >= acceleration_interval) {
                    if (a < max_acceleration) {
                        a += acceleration_increment;
                    }
                    inclination_start_time = current_time;
                }
            } else {
                inclination_start_time = current_time;
            }
...
void app_main(void)
{
    ...
    i2c_master_init();  // Add this to initialize I2C driver early
    ...
    // Register callbacks and create tasks as before
    esp_ble_gap_register_callback(gap_event_handler);
    esp_hidd_register_callbacks(hidd_event_callback);

    // Set up BLE security parameters
